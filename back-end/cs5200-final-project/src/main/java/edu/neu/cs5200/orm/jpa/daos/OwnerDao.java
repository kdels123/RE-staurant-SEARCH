...
public class OwnerDao {
	...
	// FIND Owners by criticId
	public List<Owner> findOwnerInvitesByCritic(int cid) {
		Optional<Critic> data = criticDao.findCriticById(cid);
		if (data.isPresent()) {
			Critic critic = data.get();
			return critic.getOwnerInvites();
		}
		return null;
	}

	// FIND Owners by patronId
	public List<Owner> findOwnerInvitesByPatron(int pid) {
		Optional<Patron> data = patronDao.findPatronById(pid);
		if (data.isPresent()) {
			Patron patron = data.get();
			return patron.getOwnerInvites();
		}
		return null;
	}
	...
	public void updateOwner(int id, Owner newOwner) {
		...
		if (optional.isPresent()) {
			...
			List<Restaurant> restaurants = newOwner.getRestaurants() != null
					? (List<Restaurant>) newOwner.getRestaurants()
					: currOwner.getRestaurants();
			List<Event> events = newOwner.getEvents() != null ? (List<Event>) newOwner.getEvents()
					: currOwner.getEvents();
